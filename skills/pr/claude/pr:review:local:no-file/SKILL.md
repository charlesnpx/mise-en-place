---
name: "pr:review:local:no-file"
description: "Review local branch changes without GitHub PR metadata and without writing a review file. Use when the user invokes /pr:review:local:no-file or asks to review an unpushed/local branch using local git only without saving output."
...
Do not write `~/Documents/pr-skills/reviews/PR_REVIEW.txt` or any other review file. Do not use `gh`, GitHub PR metadata, GitHub PR diffs, or GitHub API file fetches. Aside from ADO lookup when a work item ID is detected, this skill reviews local committed git history only.
